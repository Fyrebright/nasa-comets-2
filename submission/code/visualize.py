...
with open(comet_filename, 'r') as f:
    ...
    for line in lines:
        ...
        paths = []
        truths = {}
        for i in range( (len(tokens)-3)//3 ):
            images.append(tokens[2+i*3])
            paths.append(os.path.join(folder_in, tokens[0], tokens[2+i*3]))
            truths[tokens[2+i*3]] = [float(tokens[3+i*3]),tokens[4+i*3]]
        images.sort()
        paths.sort()
        seq["images"] = images
        seq["path"] = paths
